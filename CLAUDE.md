...
1. **API 키는 서버에서만.** `NEXT_PUBLIC_` 접두사가 붙은 키를 만들지 않는다.
2. **모든 라우트는 Node 런타임.** `export const runtime = "nodejs"` 를 명시한다.
   better-sqlite3와 파일 업로드가 Edge에서 동작하지 않는다.
3. **프롬프트는 코드에 넣지 않는다.** 본문은 전부 `prompts/bear-case.md`.
   코드는 이 파일을 읽어서 system 프롬프트로 넣기만 한다.
4. **출력은 자유 서술 금지.** 결과는 반드시 아래 고정 스키마를 통과해야 한다.
5. **PDF는 재업로드 금지.** 같은 파일은 SQLite에 저장된 `file_id`를 재사용한다.
